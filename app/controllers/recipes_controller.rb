class RecipesController < ApplicationController
  ...
  def search
    if params[:recipe_title_to_search]
      @recipes = Recipe.title_search(params[:recipe_title_to_search]).includes(:user,
                                                                               :favorites).page(params[:page]).per(5).reverse_order

    elsif params[:season_to_search]
      @recipes = Recipe.season_search(params[:season_to_search]).includes(:user,
                                                                          :favorites).page(params[:page]).per(5).reverse_order
